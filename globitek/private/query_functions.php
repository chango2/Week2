<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if(is_blank($state['name'])) {
      $errors[] = "State Name cannot be left blank!";
    } else if(!has_length($state['name'], array('min' => 2, 'max' => 255))) {
      $errors[] = "State Name must have at least 2 characters and max 255 characters";
    }

    if(is_blank($state['code'])) {
      $errors[] = "State Code cannot be left blank!";
    } else if(!has_length($state['code'], array('min' => 2, 'max' => 2))) {
      $errors[] = "State Code must have 2 characters";
    }
    return $errors;
  }
  ...
  function validate_territory($territory, $errors=array()) {
    if(is_blank($territory['name'])) {
      $errors[] = "Territory Name cannot be left blank!";
    } else if(!has_length($territory['name'], array('min' => 2, 'max' => 255))) {
      $errors[] = "Territory Name must have at least 2 characters and max 255 characters";
    }

    if(is_blank($territory['position'])) {
      $errors[] = "territory Position cannot be left blank!";
    } else if(!is_numeric($territory['position']) || !has_length($territory['position'], array('min' => 1, 'max' => 255))) {
  ...
  function update_territory($territory) {
    ...
    $sql .= "LIMIT 1;";    // For update_territory statments, $result is just true/false
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    } else if(!has_length($salesperson['first_name'], array('min' => 2, 'max' => 255))) {
      $errors[] = "First Name must have at least 2 characters and max 255 characters";
    }

    if(is_blank($salesperson['last_name'])) {
      $errors[] = "Last Name cannot be left blank!";
    } else if(!has_length($salesperson['last_name'], array('min' => 2, 'max' => 255))) {
      $errors[] = "Last Name must have at least 2 characters and max 255 characters";
    }

    if(is_blank($salesperson['phone'])) {
      $errors[] = "Phone number cannot be left blank!";
    } else if(!preg_match("/[0-9\-\(\) ]+/", $salesperson['phone'])) {
      $errors[] = "Wrong phone number! FIX IT";
    }
